...
export default function SaleItem({ sale }: Props) {
  const { local, sessionDate, product, salesDate, state, quantity, value } = sale;
  return (
    <div className={styles.salesItem}>
      ...
      <div className={styles.salesCol}>
        <span className={state === "Devuelta" ? styles.returned : styles.confirm}>{state}</span>
        <span>{quantity} ent</span>
        <span>{value}</span>
      </div>
    </div>
  );
